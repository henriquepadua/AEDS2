   ...
   public static void main(String[] args)throws Exception{
     ...
     for(int i = 0 ; i < inteiro;i++){
      try{
        if(line.contains("II")){
            filmes[i] = new Filme();
            filmes[i].lerHtml(leitura[i].substring(3, leitura[i].length())); 
            pegando.inserirInicio(filmes[i]);
        ...
        }else if (leitura[i].contains("I*")){
            filmes[i] = new Filme();
            String[] split = leitura[i].split(" ");
            int posicao = Integer.parseInt(split[1]);
            leitura[i].substring(6, leitura[i].length());
            pegando.Inserir(filmes[i], posicao);

        }else if(line.contains("RI")){
            pegando.removerInicio();
