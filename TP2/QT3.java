...
class Lista{
      ...
      public boolean Pesquisar(Filme[] vet, Filme x){
         boolean resp = false;
         int n = vet.length;

        for(int i = 0; i < n; i++){
          System.out.print(vet);
          if(vet[i].getNome().contains(x.getNome())){
              resp = true;
              i = n;
          }
        }
...
class QT3{
   public static void main(String[] args)throws Exception{
     MyIO.setCharset("UTF-8");
      ...
      while(!line.contains("FIM")){
        Nentrada++;
        pegando[Nentrada].inserirFim(filmes[Nentrada]);
        line = MyIO.readLine();
      }
     ...
     for(int i = 0;i < Nentrada; i++){
      if(pegando[i].Pesquisar(filmes,filmes[i]) == true)
        MyIO.println("SIM");       
      else
        MyIO.println("NAO");
     }    
